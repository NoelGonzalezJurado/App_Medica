...
export default function Header({ onSettingsClick, symptomCount, user, onSignOut }) {
  return (
      ...
      <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400 leading-none mt-0.5">
              {user?.email
                ? <span className="truncate max-w-[160px] inline-block align-bottom">{user.email}</span>
                : symptomCount === 0
                  ? 'Sin registros aún'
        ...
        <div className="flex items-center gap-1">
          ...
          {onSignOut && (
            <button
              onClick={onSignOut}
              className="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-gray-100 transition-colors text-gray-400 hover:text-rose-500"
