...
export async function exportToPDF(symptoms, analysis, patientName = 'Paciente', chartImages = {}) {
  const doc = new jsPDF();
  ...
  // ── Charts section ──
  const chartEntries = [
    { key: 'timeline', label: 'Intensidad media por día' },
    { key: 'frequency', label: 'Frecuencia de síntomas' },
    { key: 'avgInt', label: 'Intensidad media por síntoma' },
  ].filter(({ key }) => chartImages[key]);
  ...
  if (chartEntries.length > 0) {
    ...
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('Gráficas de evolución', margin, y);
    y += 8;

    const imgW = pageW - margin * 2;

    for (const { key, label } of chartEntries) {
      const aspect = await imageAspect(chartImages[key]);
      ...
      y += 4;
      doc.addImage(chartImages[key], 'PNG', margin, y, imgW, imgH);
      y += imgH + 8;
    }
  }
